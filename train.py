...
for i in range(beginner, beginner+50):
    ...
    for valid_idx in range(len(Validation)):
        ...
        if mode == 'gray':
            valid_set = valid_dataset_gray(root='F:/Gih/WDnCNN-master/testset/', dataset_name=dataset_name)
            valid_loader = data.DataLoader(valid_set, batch_size=1, num_workers=1, shuffle=False, pin_memory=False)
        else:
            valid_set = valid_dataset_color(root='F:/Gih/WDnCNN-master/testset/', dataset_name=dataset_name)
            valid_loader = data.DataLoader(valid_set, batch_size=1, num_workers=1, shuffle=False, pin_memory=False)
        PSNR = 0
        SSIM = 0
